...
In the first place, allow me to introduce the \twF results.
...
In that case, the normally high inverter sees very little disturbance; therefore its output remains unchanged.
However, when analyzing the total current waveform (medium green) of the normally low inverter, we can see that the charges are injected into the inverter, which results in charging the output capacitor $C_{L_0}$, therefore setting to one the inverter output for about 10 ns.

...
For the purpose of properly introducing the electrical models I developed for \bbi, it is required, in the first place, to focus on how integrated circuits are structured.
It involves analyzing the main structures composing an IC, such as:
\begin{itemize}
    \setlength\itemsep{-0.1em}
    \item Its power supply network, consisting of various metal levels stacked one on top of the others;
...
\end{itemize}
    ...
    This topology has the advantage of bringing a robust power delivery network inside the IC silicon.
...
%    After that, rails are drawn to connect the power to the standard-cell segments, and vias are placed between rails and straps to wire them together.
    ...
    As I have described above, the power planning of ICs creates standard-cell segments (\scs), delimited by power and ground rails.
...
%    Therefore, NMOS transistors are located in the bottom half of the standard-cell, and the PMOS are located in the top half.
    ...
    \begin{itemize}
        \setlength\itemsep{-0.1em}
        \item A \dwF substrate (Fig. \ref{subfig:dualIvx}), where the NMOS transistors are lithographed into the P-doped silicon substrate, while a N-doped area called the N-well is created inside the substrate to lithograph the PMOS transistors;
        \item A \twF substrate (Fig. \ref{subfig:tripleIvx}), where an additional buried P-doped area is manufactured inside the N-well, called the P-well, allowing to lithograph the NMOS transistors inside it instead of the substrate.
    \end{itemize}
...
%    For the purpose of illustrating the structural differences between those substrate types, I am going to use the schematics displayed in Fig. \ref{fig:dualTripleIvx} as a guide, illustrating the cross-sectional view of a logic inverter created in both a \dwF and a \twF substrate.
        ...
        A cross-sectional view of a CMOS inverter manufactured on a \dwF substrate is shown in Fig. \ref{subfig:dualIvx}.
        ...
        \begin{itemize}
            \setlength\itemsep{-0.1em}
            \item $RC_{VDD}$ represents the access resistance measured between the substrate and the bulk of the PMOS transistor through the N-well;
            \item $RC_{GND}$ is the access resistance measured between the epitaxy and the bulk of the NMOS transistor through the P-substrate.
        \end{itemize}
...
%        \textcolor{orange}{TW helps to reduce crosstalk noise.}
        ...
        Indeed, thanks to an additional silicon well, it provides an electrical insulation between areas of the IC, thus reducing substrate crosstalk noise.
        ...
        \begin{itemize}
            \setlength\itemsep{-0.1em}
            \item $R_{NW}$ is the access resistance between the substrate and the bulk of the PMOS transistor, measured through the N-well;
            \item $R_{PW}$ is the access resistance between the N-well and the bulk of the NMOS transistor, measured through the P-well.
        \end{itemize}
...
The models I developed are an improvement over the electrical models proposed by M. Dumont for \emfi \cite{mathieuEMFI}.
    ...
    \input{3_modeling/figures/mathieuFormerModels}
...
%        The dotted wires at the bottom of the substrate indicates that each substrate layer can be repeated as required to form the correct $t_{Sub}$ thickness.
        ...
        \begin{itemize}
            \setlength\itemsep{-0.1em}
            \item The region \ovalbox{1} is the \scs substrate resistive network, composed of six resistors;
            \item The region \ovalbox{2} is the P-substrate/N-well junction, modeled by a diode (DNW-PS) and its capacitance (CNW-PS), and is called the epitaxy, represented thanks to the blue net on the schematic. In addition to this, there is an access resistance from the epitaxy to the top of the N-well (VDD), representing the non-zero electrical resistance of N-doped silicon;
            \item The region \ovalbox{3} is the access resistance from the epitaxy up to the GND power rail, representing the non-zero electrical resistance of the P-doped substrate;
            \item The region \ovalbox{4P} is the model of the PMOS transistors, half of them being conducting. It consists of an access resistance RP from the VDD power-rail to the PMOS, and two capacitors, CGP being the load formed by the PMOS input capacitance, CGN the load formed by the NMOS input capacitance;
            \item The region \ovalbox{4N} is similar to \ovalbox{4P}: it is the model of the NMOS transistors, half of them being "ON". It consists of an access resistance RN from the GND power-rail to the NMOS, and two capacitors, CGP being the load formed by the PMOS input capacitors, CGN the load formed by the NMOS input capacitors;
            \item The region \ovalbox{5} and \ovalbox{5'} are the power network interconnections, represented on two metal levels, the colored ones being the first metal levels, the gray being the top metal level.
            \item Eventually, the region \ovalbox{6} represents the decoupling cells between GND and VDD power rails.
        \end{itemize}
...
%More specifically, the elementary building block of the proposed electrical model represents the average electrical behavior of several Standard-Cells, called Standard-Cell Segment (\scs).
    ...
    Among them, I can identify the coarse nature of the substrate model.
    ...
    In addition to this, the typical substrate thickness of ICs ranges from 40 µm to 1 mm, and using only two resistors to represent 700 µm of material is too wide geometric a step to fully appreciate the mechanisms at work during \bbi.
    ...
    My proposition consists in fixing the size of an elementary substrate sub model to a value of 5 µm, while allowing the user to have a certain degree of freedom concerning the elementary thickness value.
    ...
    Considering that the IC target I used for my experiments is the STM32F439VIT6, and because I have technical information concerning the inner structure of the target IC technology, I considered this information to calculate the various \scs model parameters.
    ...
    \input{3_modeling/figures/tableSimuValeur}
...
%    Because the \scs I previously introduced only describe a portion of an integrated circuit, in order to model a complete one, it is required to instantiate several \scs and to connect them with each other in a mesh grid.
    ...
    It allows mitigating human errors and enabling fast modifications to the models when required.
    ...
    I chose to use the Python scripting language to do so as I use it for every tool I write my work, therefore enabling fast support and update, while ensuring cross-script compatibility.
%    In addition to this, I decided to create an algorithm to connect as much \scs as needed to create a resulting IC.
    ...
    The actual generator Python script is annexed at the end of the manuscript (Listing \ref{listingAlgo}).
    ...
    \begin{itemize}
        ...
        \item The voltage pulse amplitude, width, rise and fall times;
        \item Various SPICE simulation settings.
    \end{itemize}
    Eventually, the program incorporates a visual inspection tool to provide a quick verification of the generated IC structure to the end-user.
...
%    After having abstractly created the models, I decided to write them using SPICE (Simulation Program with Integrated Circuit Emphasis) language.
    ...
    Indeed, they are straightforward, and it allowed me to test them independently before incorporating them into the \scs models.
    In addition to writing them, I calculated, thanks to the technology values of my IC targets, the effective substrate resistor values.
    Considering the substrate resistivity being $\rho = 0.01 \; \Omega \cdot m$, and the elementary block size being $W = 5 \; \mu m$; $L = 5 \; \mu m$; $D = 10 \; \mu m$, it is trivial to calculate each resistance value $R_i$ thanks to the following equation:
    \begin{equation}
        \label{eqn_resistivity}
        R_i = \frac{\rho \cdot l}{S}
    \end{equation}
    Knowing that $L = W \equiv LW$ and that $D = LW$, I can first write that the vertical resistances are four times the horizontal ones.
    Therefore, calculating one value of them gives the six values.
    Let us calculate the vertical resistances, which I call $RH$:
    \begin{equation}
    ...
    For example, one can reduce the number of layers by ten by adjusting the resistor values.
    It provides the advantage of a lighter simulation, which is be faster to perform, at the cost of less accuracy.

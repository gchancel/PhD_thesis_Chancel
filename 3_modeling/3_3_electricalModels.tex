...
\end{itemize}
    ...
    Thus, the lower grid brings the power close to each standard-cell segment.
...
%    Therefore, NMOS transistors are located in the bottom half of the standard-cell, and the PMOS are located in the top half.
    ...
    In these structure, there are two typical ways of lithographing the transistors inside the silicon substrate.
...
%        \textcolor{orange}{TW helps to reduce crosstalk noise.}
        ...
        Indeed, thanks to an additional silicon well, it provides an electrical isolation between areas of the IC, thus reducing crosstalk noise.
...
The models I developed are an improvement over the electrical models proposed by M. Dumont for \emfi \cite{mathieuEMFI}.
    ...
    The original Standard-Cell Segment models for \dwF and \twF substrates proposed by M. Dumont are shown in Fig. \ref{fig:dualTripleScsEmfi}.
...
%More specifically, the elementary building block of the proposed electrical model represents the average electrical behavior of several Standard-Cells, called Standard-Cell Segment (\scs).
    ...
    In addition to this, the typical substrate thickness of ICs ranges from 10 µm to 700 µm, and using only two resistors to represent 700 µm of material is a too wide geometric step to fully appreciate the mechanisms at work during \bbi.
    ...
    However, I chose a 10 µm default value for the elementary thickness as it is an optimal value, computationally speaking.
...
%    \end{itemize}
    ...
    The models I previously presented only describe a small portion of an integrated circuit.
...
%    Because the \scs I previously introduced only describe a portion of an integrated circuit, in order to model a complete one, it is required to instantiate several \scs and to connect them with each other in a mesh grid.
    ...
    Fig. \ref{fig:surfaceSplitScs} shows coarsely how it is achieved.% is a coarse illustration of what is done.
...
%    In addition to this, I decided to create an algorithm to connect as much \scs as needed to create a resulting IC.
    ...
    A simple description of the algorithm is depicted in Alg. \ref{alg:icGen}, while the actual Python script is annexed to the manuscript (\textcolor{orange}{TO-DO}).

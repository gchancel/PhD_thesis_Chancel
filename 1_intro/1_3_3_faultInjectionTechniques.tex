...
\label{chap:1;sect:fInjTech}
    ...
    Laser fault injection (\lfi), sometimes called optical fault injection, has been introduced in 2002 \cite{firstLfi}.
...
%    and is a more complex technique than \gfi.
    ...
    Although \lfi requires chip preparation, most of the time it is minimal.
    ...
    However, depending on where the laser beam travels, especially through one of the reverse biased PN junction of a transistor, where the electric field is naturally strong, the carriers deflect instead of recombining, which lead to the creation of an electric current.
...
%    \textcolor{orange}{Add more details.}
    ...
    In addition to that, cheap pulse generator can be manufactured by hand, such as very simple high-voltage transformers driven with a transistor.
    ...
    In addition to this is shown the EM susceptibility over two clock periods, in cyan, which is, in that case, equivalent to the DFF susceptibility: the fault probability is higher during the rising edge, as it is the time when the DFF sample the input.
    ...
    The targets have to be prepared in a very similar way to \lfi, with a partial removal of the package on the backside to give access to the substrate, and a potential substrate thinning.
    ...
    Considering that \emfi and \bbi both require similar voltage pulse generator, which is often the most expensive piece of equipment, the overall platform cost is lower concerning BBI.

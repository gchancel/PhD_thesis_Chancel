...
With actual \bbi platform in mind, let me introduce a typical \bbi platform model that we are going to study in details.
...
We will therefore analyze the platform's performance and point out its weaknesses.
    ...
    \begin{itemize}
        \setlength\itemsep{-0.1em}
        \item The characteristics of the voltage pulse measured at the generator output, allowing to observe how the generator behave when loaded with the transmission line and the IC;
...
%        \label{chap:2_goodPractices;sect:bbiInPractice;subsect:platformEvalCrit;subsubsect:icGndMeas}
    ...
    \end{itemize}
    These effects can also be observed on the IC ground current waveform (\textcolor{Purple}{purple}), as it is a mirror of the applied pulse due to the pure resistive nature of the IC in that model.
    ...
    Therefore, depending on the probe location and the IC substrate thickness, the generator might not see the exact same load.
